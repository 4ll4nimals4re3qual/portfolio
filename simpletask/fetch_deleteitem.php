<?php
  $_POST = json_decode(file_get_contents('php://input'), true);
  // echo $_POST['listid'];

  // データベースに接続
  try{
    $dbhost = 'mysql:host=mysql729.db.sakura.ne.jp;dbname=simpletask_db;charset=utf8';
    $dbid = 'simpletask';
    $dbpass = 'oM-6GvLb';
    $pdo = new PDO($dbhost,$dbid,$dbpass,array(PDO::ATTR_EMULATE_PREPARES=>false));
  }catch(PDOException $e){
    // exit('データベースに接続できません：'.$e->getmessage());
    exit('データベースに接続できません');
  }
  // データベース操作
  // item_table
  $sql = 'DELETE FROM `item_table` WHERE `userid`=:userid and `deskid`=:deskid and `listid`=:listid and `itemid`=:itemid';
  $stmt = $pdo->prepare($sql);
  $stmt->bindParam(':userid',$_POST['userid']);
  $stmt->bindParam(':deskid',$_POST['deskid']);
  $stmt->bindParam(':listid',$_POST['listid']);
  $stmt->bindParam(':itemid',$_POST['itemid']);
  $result = $stmt->execute();
  if(!$result):
    $stmt = null;
    $pdo = null;
    exit('サーバーエラー');
  endif;
  $stmt = null;

  // list_table
  $itemid = '';
  foreach($_POST as $key => $value):
    if(preg_match('/itemid[0-9]+/',$key)):
      $itemid .= '`'.$key.'`=:'.$key.',';//`itemid1`=:itemid1,`itemid2`=:itemid2...
    elseif(preg_match('/listid/',$key)):
      $listid = '`'.$key.'`=:'.$key;//`listid`=:listid
    endif;
  endforeach;
  $itemid = substr($itemid, 0, -1);
  $sql = 'UPDATE `list_table` SET '.$itemid.' WHERE '.$listid;
  $stmt = $pdo->prepare($sql);
  foreach($_POST as $key => $value):
    if(preg_match('/itemid[0-9]+/',$key) || preg_match('/listid/',$key)):
      $bind = ':'.$key;
      $stmt->bindValue($bind,$value);
    endif;
  endforeach;
  $stmt->execute();
  $stmt = null;

  // $itemid = '`itemid'.$_POST['index'].'`=null';//`itemid**`=null
  // $sql = 'UPDATE `list_table` SET '.$itemid.' WHERE `userid`=:userid and `deskid`=:deskid and `listid`=:listid';
  // $stmt = $pdo->prepare($sql);
  // $stmt->bindParam(':userid',$_POST['userid']);
  // $stmt->bindParam(':deskid',$_POST['deskid']);
  // $stmt->bindParam(':listid',$_POST['listid']);
  // $result = $stmt->execute();
  // if(!$result):
  //   $stmt = null;
  //   $pdo = null;
  //   exit('サーバーエラー');
  // endif;
  // $stmt = null;
  
  $pdo = null;

  
?>
